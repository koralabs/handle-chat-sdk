...
import { Direction, PreKeyPairType, SessionRecordType, SignalProtocolAddress, SignedPreKeyPairType, StorageType } from '@privacyresearch/libsignal-protocol-typescript'
...
export type StoreValue = KeyPairType | string | number | KeyPairType | PreKeyType | SignedPreKeyType | ArrayBuffer | undefined
...
export class SignalProtocolStore implements StorageType {
    ...
    remove(key: string): void {
        ...
        this._store = Object.fromEntries(
            Object.entries(this._store).filter(([k, _v]) => k !== key)
        )
    ...
    async removeSessionIdentity(address: string): Promise<void> {
        return this.remove('identityKey' + address)
    }
    ...
    async removeKeysWithIdentifier(identifier: string): Promise<void> {
        for (const id in this._store) {
            // Only remove session-related keys, not identity keys or prekeys
            if (id.startsWith('session' + identifier) || id.startsWith('identityKey' + identifier)) {
